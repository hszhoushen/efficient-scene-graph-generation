...
def anchor_target_layer(gt_boxes, anchors):
    ...
    labels = np.zeros((anchors.shape[0], cfgs.OBJ_CLASS_NUM))
    if gt_boxes.shape[0]:
        # [N, M]
    ...
    return np.array(labels, np.float32), np.array(target_delta, np.float32), np.array(anchor_states, np.float32)
