...
def only_draw_boxes(img_batch, boxes, in_graph=True):
    ...
    img_tensor_with_boxes = tf.py_func(draw_box_in_img.draw_boxes_with_label_and_scores,
                                       inp=[img_tensor, boxes, labels, scores, in_graph],
                                       Tout=tf.uint8)
...
def draw_boxes_with_scores(img_batch, boxes, scores, in_graph=True, detection=False):
    ...
    img_tensor_with_boxes = tf.py_func(draw_box_in_img.draw_boxes_with_label_and_scores,
                                       inp=[img_tensor, boxes, labels, scores, in_graph, detection],
                                       Tout=[tf.uint8])
    img_tensor_with_boxes = tf.reshape(img_tensor_with_boxes, tf.shape(img_batch))
    return img_tensor_with_boxes


def draw_boxes_with_categories(img_batch, boxes, labels, in_graph=True, detection=False):
    boxes = tf.stop_gradient(boxes)
...
def draw_boxes_with_categories_and_scores(img_batch, boxes, labels, scores, in_graph=True, detection=False):
    boxes = tf.stop_gradient(boxes)
    scores = tf.stop_gradient(scores)
