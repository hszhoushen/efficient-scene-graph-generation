...
def draw_label_with_scores(draw_obj, box, label, score, color, detection=False):
    x, y = box[0], box[1]
    draw_obj.rectangle(xy=[x, y, x + 60, y + 10],
                       fill=color)

    if detection:
        LABEL_NAME_MAP=obj_class_names
    else:
        LABEL_NAME_MAP=rel_class_names
...
def draw_boxes_with_label_and_scores(img_array, boxes, labels, scores, in_graph=True, detection=False, color=None):
    if in_graph:
    ...
    img_array = np.array(img_array * 255.0 / np.max(img_array), dtype=np.uint8)
    ...
    for box, a_label, a_score in zip(boxes, labels, scores):
        ...
        if a_label != NOT_DRAW_BOXES:
            ...
            if color is not None:
                draw_a_rectangel_in_img(draw_obj, box, color=color, width=3)
            else:
            ...
            if a_label == ONLY_DRAW_BOXES:  # -1
                ...
                draw_label_with_scores(draw_obj, box, a_label, a_score, color='White', detection=detection)

    out_img_obj = Image.blend(raw_img_obj, img_obj, alpha=0.7)

    return np.array(img_obj)#np.array(out_img_obj)
