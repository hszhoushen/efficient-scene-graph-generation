...
from libs.box_utils import anchor_utils, generate_anchors
...
class DetectionNetwork(object):
    ...
    def __init__(self, base_network_name, is_training, return_featuremap=False):

        self.base_network_name = base_network_name
        self.is_training = is_training
        self.return_featuremap=return_featuremap
    ...
    def rpn_cls_net(self, inputs, scope_list, reuse_flag, level):
        ...
        rpn_box_scores = slim.conv2d(rpn_conv2d_3x3,
                                     num_outputs=cfgs.OBJ_CLASS_NUM * self.num_anchors_per_location,
                                     kernel_size=[3, 3],
                                     stride=1,
                                     weights_initializer=cfgs.SUBNETS_WEIGHTS_INITIALIZER,
        ...
        rpn_box_scores = tf.reshape(rpn_box_scores, [-1, cfgs.OBJ_CLASS_NUM],
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_batch):
        ...
        if self.is_training:
            with tf.variable_scope('build_loss'):
                ...
                self.add_anchor_img_smry(input_img_batch, anchors, anchor_states)
        ...
        if self.is_training:
            ...
            if self.return_featuremap:
                return boxes, scores, category, feature_pyramid
            else:
                return boxes, scores, category

    def get_restorer(self):
        checkpoint_path = tf.train.latest_checkpoint(os.path.join(cfgs.TRAINED_CKPT, cfgs.DETECTION_VERSION))
