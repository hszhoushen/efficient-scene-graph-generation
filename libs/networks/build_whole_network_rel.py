...
class RelDetectionNetwork(object):
    ...
    def build_whole_rel_network(self, input_img_batch, R_matrix, gt_boxes):
        ...
        # self.det_restorer, self.det_restore_ckpt = self.det_net.get_restorer()

        # 2. union boxes, subjec/object boxes, rel_labels
        with tf.variable_scope('build_union_boxes'):
            if self.is_training:
                sub_boxes,obj_boxes,rel_boxes,rel_labels = tf.py_func(func=full_connection_graph,
                                                                      inp=[R_matrix, gt_boxes],
                                                                      Tout=[tf.float32, tf.float32, tf.float32, tf.float32])
            else:
                sub_boxes,obj_boxes,sub_categorys,obj_categorys,sub_scores,obj_scores,rel_boxes = tf.py_func(func=full_connection_graph_inference,
        ...
        if self.is_training:
            with tf.variable_scope('build_loss'):
                cls_loss = losses.focal_loss(rel_labels, rel_pred, None, alpha=0.75)
            cls_pred = tf.argmax(rel_prob,axis=1)
            cls_gt = tf.argmax(rel_labels,axis=1)
            acc = tf.reduce_mean( tf.to_float( tf.equal(cls_pred,cls_gt) ) )
            tf.summary.scalar('accuracy', acc)

        with tf.variable_scope('postprocess_detctions'):
            if self.is_training:
                rel_boxes, rel_scores, rel_labels, sub_boxes, obj_boxes=\
                    postprocess_detctions(rel_cls_prob=rel_prob,  
                                          rel_boxes=rel_boxes,
                                          sub_boxes=sub_boxes,
                                          obj_boxes=obj_boxes,
                                          sub_categorys=None,
                ...
                rel_boxes, rel_scores, rel_labels, sub_boxes, obj_boxes = tf.stop_gradient(rel_boxes), tf.stop_gradient(rel_scores), tf.stop_gradient(rel_labels), tf.stop_gradient(sub_boxes), tf.stop_gradient(obj_boxes)
            else:
                rel_boxes, rel_scores, rel_labels, sub_boxes, sub_scores, sub_labels, obj_boxes, obj_scores, obj_labels=\
                    postprocess_detctions(rel_cls_prob=rel_prob,  
                                          rel_boxes=rel_boxes,
                                          sub_boxes=sub_boxes,
                                          obj_boxes=obj_boxes,
                                          sub_categorys=sub_categorys,
                                          obj_categorys=obj_categorys,
                                          sub_scores=sub_scores,
                                          obj_scores=obj_scores,
                                          is_training=self.is_training)
    
        if self.is_training:
            return rel_boxes, rel_scores, rel_labels, sub_boxes, obj_boxes, cls_loss#, rel_prob
        else:
            return rel_boxes, rel_scores, rel_labels, sub_boxes, sub_scores, sub_labels, obj_boxes, obj_scores, obj_labels#, rel_prob
